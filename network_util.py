...
def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'lambda':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.epoch_count - opt.lr_decay_iters) / float(opt.niter_decay + 1)
            return lr_l
...
def mixup_data(x, y, num_classes,alpha=1.0, label_smoothing=0.1,device='cuda'):
    ...
    y1 = one_hot(y, num_classes, on_value=on_value, off_value=off_value, device=device)
    ...
    y2 = one_hot(y.flip(0), num_classes, on_value=on_value, off_value=off_value, device=device)
    return mixed_x,  lam * y1 + (1 - lam) * y2, y1, y2, lam
    # return mixed_x,  lam * y_a + (1 - lam) * y_b, y_a, y_b, lam
